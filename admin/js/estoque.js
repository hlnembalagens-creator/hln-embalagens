...
async function mostrarRevisao(dados) {
  ...
  document.getElementById('rev-cnpj').value = forn.cnpj || '';
  document.getElementById('rev-razao-social').value = forn.razao_social || '';
  document.getElementById('rev-nome-fantasia').value = forn.nome_fantasia || '';
  document.getElementById('rev-logradouro').value = forn.logradouro || '';
  document.getElementById('rev-numero').value = forn.numero || '';
  document.getElementById('rev-bairro').value = forn.bairro || '';
  document.getElementById('rev-cep').value = forn.cep || '';
  document.getElementById('rev-municipio').value = forn.municipio || '';
  document.getElementById('rev-uf').value = forn.uf || '';

  var badgeEl = document.getElementById('import-fornecedor-badge');
  var cnpjNormalizado = normalizarCnpjEstoque(forn.cnpj);
  if (cnpjNormalizado) {
    var { data: existente } = await supabaseClient.from('clientes').select('id, razao_social').eq('cnpj_cpf', forn.cnpj).maybeSingle();
    badgeEl.innerHTML = existente
      ? '<span class="badge badge-ok">Já cadastrado: ' + existente.razao_social + '</span>'
      : '<span class="badge badge-warning">Novo fornecedor — vai ser criado</span>';
  } else {
    badgeEl.innerHTML = '<span class="badge badge-warning">Sem CNPJ identificado — confira antes de salvar</span>';
  }
...
document.getElementById('import-confirmar-btn').addEventListener('click', async function () {
  ...
  try {
    // 1) Fornecedor — atualiza se já existe (por CNPJ), cria se não existe.
    ...
    if (cnpj) {
      var { data: existente } = await supabaseClient.from('clientes').select('id, razao_social, nome_fantasia').eq('cnpj_cpf', cnpj).maybeSingle();
